...
export async function bulkAddCourses(courses: any[]) {
    const { data, error } = await supabaseAdmin.from('courses').insert(courses).select()
    if (error) return { error: error.message }
    return { data }
}

export async function bulkAddProgrammes(programmes: any[]) {
    const { data, error } = await supabaseAdmin.from('programmes').insert(programmes).select()
    if (error) return { error: error.message }
    return { data }
}
